...
#=======================================================================================================================================
parser = argparse.ArgumentParser(description='LPAE for DIV2K')
...
parser.add_argument('--traindata', default='/home/sjhan/datasets/div2k/train_HR', help='Path for train dataset')
parser.add_argument('--testdata', default='/home/sjhan/datasets/div2k/valid_HR', help='Path for validation dataset')

parser.add_argument('--model_save', default='./model_save', help='Path to save trained model')
parser.add_argument('--model_name', default='wae_change_structure_div2k_rec_l1.pth', help='Name for trained model')
...
def train_reset(batchSize, dataroot, workers=args.workers, rancrop=True, rancropsz=192, horiflip=True, vertflip=True):
    trans_list = []
    if rancrop:
...
def test_reset(batchSize, dataroot, workers=args.workers, resize=True, resizesz=192, rancrop=False, rancropsz=192):
    trans_list=[]
    if resize:
...
#=======================================================================================================================================
testloader = test_reset(args.testbatch, args.testdata, workers=args.workers, resize=False, resizesz=192, rancrop=True, rancropsz=192)
lamb = 1
gamm = 0.8
...
if args.test_only is True :
    ...
    with torch.no_grad():
        ...
        for testiter, testdata in enumerate(testloader):
            ...
            losstest = loss_l1(tinput, testout) + lamb * loss_MSE(test_d, 0) + gamm * loss_MSE(test_input_down, test_c)
            losstest_sum += losstest.item()

            mse = loss_MSE(tinput, testout)
            psnr = -10.0 * math.log10(mse.item())
            avg_psnr += psnr

        tinfo = "testloss: {:.5f}, avg_psnr: {:.4f}".format(losstest_sum/len(testloader), avg_psnr/len(testloader))
        print(tinfo)
...
#=======================================================================================================================================
    ...
    for epoch in range(args.nepoch):        
        loss_sum = 0
        trainloader = train_reset(args.trainbatch, args.traindata, workers=args.workers, rancrop=True, rancropsz=192, horiflip=True, vertflip=True)
        ...
        if epoch % 2 == 0 or epoch == (args.nepoch - 1):
            ...
            tinfo = "testloss: {:.5f}".format(losstest_sum/len(testloader))
            print(tinfo)
