...
#=======================================================================================================================================
parser = argparse.ArgumentParser(description='WAE for ImageNet')
...
parser.add_argument('--model_name', default='wae_papersetting_imagenet.pth', help='Name for trained model')
...
if args.test_only is True :
    ...
    with torch.no_grad():
        ...
        for testiter, testdata in enumerate(testloader):
            ...
            losstest = loss_MSE(tinput, testout) + lamb * loss_MSE(test_d, 0)
...
#=======================================================================================================================================
    ...
    for epoch in range(args.nepoch):    
        ...
        #--------------train-------------
            ...
            optimizer.zero_grad()
            x_L, x_H, out = net(imgs)

            loss = loss_MSE(imgs, out) + lamb * loss_MSE(x_H, 0)
            loss.backward()
            optimizer.step()
        ...
        if epoch % 2 == 0 or epoch == args.nepoch-1:
            ...
            net.eval()
            for testiter, testdata in enumerate(testloader):
                tdata, tlabels = testdata
                if use_cuda:
                    tdata = tdata.to(device)

                test_L, test_H, testout = net(tdata)

                losstest = loss_MSE(tdata, testout) + lamb * loss_MSE(test_H, 0)
                losstest_sum += losstest.item()

            tinfo = "testloss: {:.4f}".format(losstest_sum/len(testloader))
            print(tinfo)
