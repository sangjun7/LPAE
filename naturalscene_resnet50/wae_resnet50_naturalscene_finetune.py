...
#=======================================================================================================================================
parser = argparse.ArgumentParser(description='WAE + ResNet50 finetune for NaturalScene')
parser.add_argument('--nepoch', default=30, type=int, help='Number of training epochs')
...
parser.add_argument('--model_name', default='resnet50_of_papersetting_naturalscene_combine_finetune.pth', help='Name for trained model')
parser.add_argument('--pretrained_AE', default='./model_save/wae_papersetting_naturalscene.pth', help='pretrained LPAE parameters')
parser.add_argument('--pretrained_resnet', default='./model_save/resnet50_of_papersetting_naturalscene_combine.pth', help='pretrained ResNet parameters')
...
class resnet50(nn.Module):
    ...
    def forward(self, x):
        ...
        c = self.layer2_1(x)
        d = self.layer2_2(x)
        c_up = self.layer3_1(c)
        d_up = self.layer3_2(d)
        out =  d_up + c_up
...
if args.test_only is True :
    ...
    with torch.no_grad():
        ...
        for testiter, testdata in enumerate(testloader):
            ...
            testloss_wae = loss_MSE(tinput, testout) + loss_MSE(test_d, 0)
...
#=======================================================================================================================================
    ...
    pretrained_wae_dict = {k: v for k, v in pretrained_wae.items() if k in resnet_dict}
    pretrained_res_dict = {k: v for k, v in pretrained_res.items() if k in resnet_dict}
    resnet_dict.update(pretrained_wae_dict)
    resnet_dict.update(pretrained_res_dict)
    ...
    for epoch in range(args.nepoch): 
        ...
        #--------------train-------------
            ...
            optimizer.zero_grad()
            sL, sC, s, input_c, input_d, inputout = resnet(inputs)

            loss_wae = loss_MSE(inputs, inputout) + loss_MSE(input_d, 0)
        ...
        if epoch % 4 == 0 or epoch == (args.nepoch - 1):
            ...
            for testiter, testdata in enumerate(testloader):
                ...
                tsL, tsC, ts, test_c, test_d, testout = resnet(tinput)
